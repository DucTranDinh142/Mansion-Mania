...
public class UI_SkillToolTip : UI_ToolTip
{
    ...
    public void ShowToolTip(bool show, RectTransform targetRect, UI_TreeNode node)
    {
        ...
        string requirements = node.isLocked ? skillLockedText : GetRequirements(node.skillData.upgradeData.cooldown,node.skillData.cost, node.neededNodes, node.conflictNode);

        skillRequirements.text = requirements;
    }

    private string GetRequirements(float cooldown,int skillCost, UI_TreeNode[] neededNodes, UI_TreeNode[] conflictNodes)
    {
        StringBuilder stringBuilder = new StringBuilder();

        stringBuilder.AppendLine($"Hồi chiêu: {cooldown} giây");
        stringBuilder.AppendLine("Yêu cầu mở khóa:");

        string costColor = skillTree.EnoughSkillPoints(skillCost) ? metConditionHex : notMetConditionHex;
        string costText = $"*{skillCost} điểm kỹ năng";
